      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3.5 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 sm:gap-4">
          {showTimer && remainingSeconds !== undefined && (
            <div
              className={`flex items-center gap-2 px-3 sm:px-4 py-1.5 rounded-xl font-bold text-sm sm:text-base border transition ${
                isTimeCritical
                  ? "bg-red-50 text-red-600 border-red-300 animate-pulse"
                  : "bg-slate-900 text-white border-slate-800"
              }`}
            >
              <Clock className={`w-4 h-4 ${isTimeCritical ? "text-red-600" : "text-red-400"}`} />
              <span className="font-mono tracking-wider">{formatTime(remainingSeconds)}</span>
            </div>
          )}

          {studentName ? (
            <div className="flex items-center gap-1.5 bg-gray-100 text-gray-800 px-3 py-1.5 rounded-full text-xs font-medium border border-gray-200">
              <User className="w-3.5 h-3.5 text-gray-500" />
              <span className="max-w-[120px] sm:max-w-[180px] truncate">{studentName}</span>
            </div>
          ) : (
            <div className="hidden md:flex items-center gap-1.5 text-xs text-emerald-700 bg-emerald-50 border border-emerald-200 px-3 py-1.5 rounded-full font-medium">
